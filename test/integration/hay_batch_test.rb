...
class Hay::Batch::IntegrationTest < Test::Unit::TestCase
  class Consumer
  ...
  test 'consuming and flowing' do
    ...
    flow = Hay::Batch::Task::Flow.new("between" => [terminal_task], "after" => [terminal_task])
    ...
    assert_equal 1, agent.messages.length
    assert_kind_of RemoteTask, agent.messages.first.task.__getobj__
    assert_kind_of Hay::Batch::Task::Decorator, agent.messages.first.task
  end
